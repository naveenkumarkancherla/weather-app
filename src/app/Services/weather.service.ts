...
export class WeatherService {
  ...
  fillTemperatureDataModel(){
    ...
    this.temperatureData.time = `${String(hours).padStart(2, '0')}:${minutes} ${ampm}`;
  ...
  fillTodaysHighlight(){
    ...
    const formatTimeTo12Hour = (time: string): string => {
      let [hours, minutes] = time.slice(11, 16).split(':').map(Number);
      hours = hours % 12;
      hours = hours ? hours : 12;
    ...
    this.todaysHighlight.sunrise = formatTimeTo12Hour(this.weatherDetails['v3-wx-observations-current'].sunriseTimeLocal);
    this.todaysHighlight.sunset = formatTimeTo12Hour(this.weatherDetails['v3-wx-observations-current'].sunsetTimeLocal);
